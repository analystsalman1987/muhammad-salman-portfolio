...
export function Software({ software }: SoftwareProps) {
  ...
  const allExpanded =
    software.length > 0 && expandedIds.size === software.length;
  ...
  const getSoftwareLogo = (name: string) => {
    ...
    // Delta Financial / Delta Software
    if (
      lower.includes('delta') ||
      lower.includes('delta financial')
    ) {
      return {
        src: deltaLogo,
        alt: 'Delta Software',
        className: 'h-7 sm:h-7.5 w-auto max-w-[125px] object-contain',
  ...
  return (
    <section
      id="software"
      className="py-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-colors"
    >
